...
function clickHandler(event) {
    ...
    let typeOfProduct = event.target.getAttribute('data-type');
    showCategory(typeOfProduct);
}
...
function showCategory(category) {
    let categoryOfProduct = products[category];
    let markupForProducts = '';
    categoryOfProduct.forEach(function (product) {
        markupForProducts += getProductMarkup(product)
    })
    productsBox.insertAdjacentHTML('afterbegin', markupForProducts);
}
